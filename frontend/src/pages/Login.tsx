        ...
        <div className="min-h-screen w-full flex bg-brand-cream text-stone-900">
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 overflow-y-auto">
                <div className="w-full max-w-md bg-white p-10 rounded-[40px] shadow-sm border border-stone-900/5 my-8">
                    ...
                    {errorMsg && (
                        <div className={`mb-6 p-4 rounded-xl text-sm font-medium ${errorMsg.includes('successful') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
                            {errorMsg}
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <>
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-4">
                                        <label className={`flex-1 flex items-center justify-center gap-2 py-3 border-2 rounded-xl cursor-pointer transition-all ${role === 'customer' ? 'border-brand-primary bg-brand-primary/5 text-brand-primary' : 'border-stone-200 text-stone-500 hover:border-brand-primary/50'}`}>
                                            <input type="radio" className="hidden" name="role" value="customer" checked={role === 'customer'} onChange={() => setRole('customer')} />
                                            <span className="font-bold">Customer</span>
                                        </label>
                                        <label className={`flex-1 flex items-center justify-center gap-2 py-3 border-2 rounded-xl cursor-pointer transition-all ${role === 'chef' ? 'border-brand-primary bg-brand-primary/5 text-brand-primary' : 'border-stone-200 text-stone-500 hover:border-brand-primary/50'}`}>
                                            <input type="radio" className="hidden" name="role" value="chef" checked={role === 'chef'} onChange={() => setRole('chef')} />
                        ...
                        <div>
                            ...
                            {!isLogin && <p className="text-xs text-stone-500 mt-2 ml-1">Must be at least 6 characters long</p>}
                        </div>
